...
def start(update, context):
    update.message.reply_text(
        "Привет! Это бот с игрой в конфеты! Нажмите info чтоб прочитать правила или play чтоб начать играть",
        reply_markup=markup)


def play(update, context):
    update.message.reply_text("С каким ботом будете играть, с обычным (simple) или умным (clerer)?",
                              reply_markup=bot_markup)
    # return 1


def simple_bot(update, context):
    update.message.reply_text("Вы выбрали обычного бота. Введите количество конфет с которыми будете играть",
...
def clever_bot(update, context):
    global flag
    flag = False
    update.message.reply_text("Вы выбрали умного бота. Введите количество конфет с которыми будете играть",
                              reply_markup=stop_markup)
    return 1
...
def info(update, context):
    update.message.reply_text(
        "Правила игры с конфетами: Для начала необходимо ввести общее количество конфет. Далее по очереди из общей кучи забираете от 1 до 28 конфет. Побеждает тот, кто сможет забрать все последние конфеты.")
